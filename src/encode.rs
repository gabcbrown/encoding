...
fn find_next_match(
    ...
    let character: Option<u8> = None; // character after match
                                      // let mut matched: Option<_> = None; // TODO: doesn't need to be mut, should only assign once
    ...
    for j in (1..min(buffer_window, lookahead_window) + 1).rev() {
        ...
        if let Some(matched) = find(
            &input[starting_position - buffer_window..starting_position],
            &input[starting_position..starting_position + j],
        ) {
            return Some((matched, j, input[starting_position].clone()));
        }
    }
...
fn find(input1: &[u8], input2: &[u8]) -> Option<usize> {
    if input1.len() < input2.len() {
        None
    } else {
        for i in 0..input1.len() - input2.len() + 1 {
